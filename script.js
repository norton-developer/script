...
// === TOAST УБРАН - ОН КРАШИТ ===
// Вместо toast() просто console.log()
function toast(msg) { 
...
waitForModule(function(baseAddr) {
    ...
    if (addActionAddr) {
        ...
        console.log("[-] ОШИБКА: addAction не найден!");
    }
            ...
            onEnter: function(args) {
                ...
                if (box_buttonobjmenu && ptr(box_buttonobjmenu).toString() === ptr(btn).toString()) {
                    ...
                    if (player_id) {
                        ...
                        console.log("[B.O.X] ID не найден");
                    }
    ...
    if (clientsendAddr) {
        Interceptor.attach(clientsendAddr, { 
            ...
            // === GIFT COMMANDS ===
                ...
                if (cmd === "!gifton") {
                    GIFT_ENABLED = true;
                    giftLog("Включено: " + GIFT_TARGET);
                }

            // === ANIMATION ===
                if (cmd === "!guitar") {
                    nextNet = { gr: "skygacha26_guitar_off", at: "PlayGuitNew1" };
                    nextVisual = "PlayGuitNew1";
                    isLocked = true;
                    validVTable = null;
                    console.log("[+] ГИТАРА! Нажми на предмет.");
                }
                ...
                if (cmd === "!tree") {
                    ...
                    isLocked = true;
                    validVTable = null;
                    console.log("[+] Ёлка! Нажми на предмет.");
                }
                ...
                if (cmd === "!rep" && uid) {
                    repeatCount = Math.min(parseInt(uid) || 10, 100);
                }

                if (cmd === "!dupe" && uid) {
                    duplicateRequest(parseInt(uid) || 20, 200);
                }
                if (cmd === "!dupe" && !uid) {
                    duplicateRequest(20, 50);
                }
                ...
                if (cmd === "!xmas_dupe" && uid) {
                    ...
                    console.log('[!] Нажми предмет для старта!')
                }
            ...
            // === SLOTS ===
                if (cmd === "!save" && uid) {
                    ...
                    if (nextNet) {
                        ...
                        console.log("[+] " + uid + " = " + nextNet.at);
                    }
                }

                if (cmd === "!del" && uid && savedSlots[uid]) {
                    delete savedSlots[uid];
                    console.log("[+] Удалено");
                }
                ...
                if (cmd === "!slots") {
                    var list = "";
                    for (var s in savedSlots) list += s + ": " + savedSlots[s].at + "\n";
                    console.log(list || "Пусто");
                }

                if (cmd === "!anim" && uid) {
                    playLocalAnimation(uid);
                }

            // === RELATIONS ===
                if (cmd === "!tofriend" && uid) {
                    chainToFriend(uid, 2500);
                }
    ...
    console.log("[+] GIFT HACK: ВКЛ (цель: " + GIFT_TARGET + ")");
    console.log("[+] B.O.X кнопка в меню аватара");
    console.log("");

});

} // конец initScript()        "cryPose": "cry",
        "angryPose": "angry"
    };
    ...
    function giftLog(msg) { 
        console.log("\x1b[31m[GIFT]\x1b[0m " + msg); 
    }
    ...
    function smartFinishAll() {
        ...
        if (!finishAddr) return;
        var finishWork = new NativeFunction(finishAddr, 'void', ['pointer']);
        var uniqueObjects = [...new Set(workObjects)];
        var count = 0;
        uniqueObjects.forEach(function(objAddr) {
            try {
                if (objAddr.readPointer() !== null) { 
                    finishWork(objAddr);
                    count++;
                }
            } catch (e) {}
    ...
    function sendRelStatus(uid, status) {
        if (!globalProcessor || !changeStatusCtor) {
            toast("Сделай действие через UI!");
            return false;
        }
        try {
            var requestBuf = Memory.alloc(256);
    ...
    waitForModule(function(baseAddr) {
            ...
            Interceptor.attach(ctorAddr, {
                onEnter: function(args) {
                    ...
                    if (repeatCount > 0) {
                        ...
                        repeatCount = 0;
                        setTimeout(function() { duplicateRequest(count, 50); }, 100);
                    }
                }
            });
        }
            ...
            Interceptor.attach(customActionAddr, {
                onEnter: function(args) {
                    ...
                    if (box_buttonobjmenu && ptr(box_buttonobjmenu).toString() === ptr(btn).toString()) {
                        ...
                        if (player_id) {
                            console.log("[B.O.X] Цель: " + player_id);
                            toast("[B.O.X] Добавляю: " + player_id);
                            chainToFriend(player_id, 2500);
                        } else {
                            toast("[B.O.X] ID не найден");
                        }
                        box_buttonobjmenu = null;
                    }
                }
            ...
            Interceptor.attach(clientsendAddr, { 
                ...
                // === ANIMATION ===
                    ...
                    if (cmd === "!xmas_dupe" && uid) {
                        ...
                        validVTable = null;
                        duplicateRequest(parseInt(uid) || 10, 800);
                        toast('[!] Нажми предмет для старта!')
                    }
        ...
        console.log("[+] B.O.X кнопка в меню аватара");
        console.log("");
        console.log("=== GIFT КОМАНДЫ ===");
        console.log("!gift <id>   - Сменить цель");
        console.log("!giftoff     - Выключить");
        console.log("!gifton      - Включить");
        console.log("");
